import { NextRequest, NextResponse } from "next/server";

export const runtime = "nodejs";

const BACKEND_URL = process.env.BACKEND_API_URL ?? "http://localhost:4000/api/v1";
const COOKIE_NAME = "hsakaa_person_session";

export async function GET(request: NextRequest) {
  const token = request.cookies.get(COOKIE_NAME)?.value;
  if (!token) return NextResponse.json({ verified: false }, { status: 401 });

  try {
    const backend = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/memory-verification/session`, {
      headers: { Accept: "application/json", "x-memory-session": token },
      cache: "no-store",
    });

    if (!backend.ok) {
      const response = NextResponse.json({ verified: false }, { status: 401 });
      response.cookies.set(COOKIE_NAME, "", { path: "/", maxAge: 0 });
      return response;
    }

    return new NextResponse(await backend.text(), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("HSAKAA session restore error:", error);
    return NextResponse.json({ message: "Unable to restore verification right now." }, { status: 500 });
  }
}
