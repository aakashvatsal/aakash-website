...
export function NowPage({ now: initialNow, history = [] }: NowPageProps) {
  const [now, setNow] = useState<NowStatus | null>(initialNow);
  ...
  const refreshNow = useCallback(async () => {
    try {
      ...
      setNow((await response.json()) as NowStatus | null);
    } catch {
      // Keep the last known status if the network briefly fails.
    }
  }, []);
  ...
  if (!now) {
    return <EmptyNowState />;
  }

  const activityType = formatLabel(now.activityType);

  const availability = formatLabel(now.availability);

  const mood = formatLabel(now.mood);

  const startedAt = formatDate(now.startedAt);

  const updatedAt = formatDate(now.updatedAt);

  const relativeStartedAt = formatRelativeTime(now.startedAt);

  const hasBuilding = Boolean(
    now.building?.companyName ||
    now.building?.projectName ||
    now.building?.currentWork,
  );

  const hasReading = Boolean(
    now.reading?.title ||
    now.reading?.author ||
    now.reading?.currentThought ||
    now.reading?.progressPercentage !== undefined,
  );

  const hasHealth = Boolean(
    now.health?.activity ||
    now.health?.summary ||
    now.health?.workoutDurationMinutes !== undefined ||
    now.health?.steps !== undefined ||
    now.health?.sleepHours !== undefined ||
    now.health?.recoveryScore !== undefined ||
    now.health?.strainScore !== undefined ||
    now.health?.heartRateVariabilityMs !== undefined ||
    now.health?.restingHeartRateBpm !== undefined ||
    now.health?.energyScore !== undefined,
  );

  const hasCapacity =
    now.energyScore !== undefined || now.focusScore !== undefined;

  const recentHistory = history
    .filter((item) => item._id !== now._id)
    .slice(0, 6);
    ...
    <main className="min-h-screen bg-[#030608] text-white">
      <section className="relative overflow-hidden pb-16 pt-28">
        ...
        <Container className="relative">
          ...
          <BodyText className="mt-8 max-w-3xl">
            A live window into what I&apos;m building, thinking, reading and
            paying attention to.
          </BodyText>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-x-4 gap-y-3 text-sm">
            {relativeStartedAt && (
              <div className="flex items-center gap-2 text-white/35">
                ...
                <span>Started {relativeStartedAt}</span>
              </div>
            )}

            {now.showLocation && now.locationName && (
              <>
      ...
      <section className="pb-32">
        <Container>
          ...
          {(hasBuilding || now.thinking || now.writing) && (
            <div className="mt-24">
              <SectionHeader
                eyebrow="In my head"
              ...
              <div className="mt-10 grid gap-5 lg:grid-cols-12">
                {hasBuilding && <BuildingPanel now={now} />}
                ...
                >
                  {now.thinking && <ThoughtPanel text={now.thinking} />}

                  {now.writing && <WritingPanel text={now.writing} />}
                </div>
              </div>
            </div>
          )}

          {(hasReading || hasHealth || hasCapacity) && (
            <div className="mt-24">
              <SectionHeader
              ...
              <div className="mt-10 grid gap-5 lg:grid-cols-12">
                {hasReading && <ReadingPanel now={now} />}

                {hasHealth && <HealthPanel now={now} />}

                {hasCapacity && <CapacityPanel now={now} />}
              </div>
            </div>
          )}
          ...
          {recentHistory.length > 0 && (
            <div className="mt-24">
              ...
              <NowTimeline history={recentHistory} />
            </div>
          )}
          ...
          <div className="mt-24 flex flex-col gap-4 border-t border-white/10 pt-8 text-sm text-white/30 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              ...
              <span>This page reflects my current public context.</span>
            </div>
      ...
      <div className="grid lg:grid-cols-[1fr_320px]">
        <div className="relative px-7 py-10 sm:px-10 lg:px-12 lg:py-14">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {startedAt && (
              <span className="text-xs text-white/25">{startedAt}</span>
            )}
          </div>

          <h1 className="mt-8 max-w-4xl text-4xl font-black leading-[1.02] tracking-[-0.055em] sm:text-5xl lg:text-6xl">
            {now.headline || now.activity}
          </h1>

          {now.headline && now.activity !== now.headline && (
        ...
        <div className="border-t border-white/10 bg-white/[0.012] p-7 lg:border-l lg:border-t-0 lg:p-9">
          ...
          <div className="mt-6 space-y-7">
            {availability && (
              <StateValue label="Availability" value={availability} />
            )}

            {now.showMood && mood && <StateValue label="Mood" value={mood} />}

            {now.showLocation && now.locationName && (
              <StateValue
                label="Location"
                value={now.locationName}
                detail={
                  now.locationType ? formatLabel(now.locationType) : undefined
                }
              />
            )}
          </div>
...
function BuildingPanel({ now }: { now: NowStatus }) {
...
function ThoughtPanel({ text }: { text: string }) {
  return (
...
function WritingPanel({ text }: { text: string }) {
  return (
    <SpotlightCard className="rounded-[30px] p-7 lg:min-h-[210px]">
      <div className="flex items-center gap-3">
        <IconBox>
...
function ReadingPanel({ now }: { now: NowStatus }) {
  const reading = now.reading;

  const progress = reading?.progressPercentage;
  ...
  return (
    <SpotlightCard className="rounded-[32px] p-8 lg:col-span-5">
      ...
      <h3 className="mt-10 text-3xl font-black leading-tight tracking-[-0.045em]">
        {reading?.title || "Current reading"}
      </h3>

      {reading?.author && (
        <p className="mt-3 text-sm text-white/35">by {reading.author}</p>
      )}
...
function HealthPanel({ now }: { now: NowStatus }) {
  const health = now.health;

  const metrics = [
    typeof health?.recoveryScore === "number"
      ? {
          label: "Recovery",

          value: `${Math.round(health.recoveryScore)}%`,

          status: getRecoveryStatus(health.recoveryScore),
        }
      : null,
    ...
    typeof health?.sleepHours === "number"
      ? {
          ...
          value: formatHours(health.sleepHours),

          status: "neutral" as const,
        }
      : null,

    typeof health?.strainScore === "number"
      ? {
          label: "Strain",

          value: health.strainScore.toFixed(1),

          status: "neutral" as const,
        }
      : null,

    typeof health?.heartRateVariabilityMs === "number"
      ? {
          label: "HRV",

          value: `${health.heartRateVariabilityMs.toFixed(1)} ms`,

          status: "neutral" as const,
        }
      : null,

    typeof health?.restingHeartRateBpm === "number"
      ? {
          label: "Resting HR",
    ...
    typeof health?.steps === "number"
      ? {
          ...
          value: health.steps.toLocaleString("en-IN"),
...
function CapacityPanel({ now }: { now: NowStatus }) {
  return (
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/35">Mental bandwidth</p>
            </div>
          </div>
        </div>

        <ScoreBar label="Energy" value={now.energyScore} />

        <ScoreBar label="Focus" value={now.focusScore} />
      </div>
    </SpotlightCard>
  );
}
      ...
      <div className="flex items-end justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-3xl font-black tracking-[-0.05em]">
            ...
            <span className="ml-1 text-base text-white/25">/10</span>
          </p>
        </div>
      </div>
...
function NowTimeline({ history }: { history: NowStatus[] }) {
  return (
    <div className="mt-10">
        ...
        >
          <div className="hidden pt-1 text-right md:block">
            ...
            <p className="mt-1 text-[10px] text-white/15">
              {formatTimelineTime(item.startedAt)}
            </p>
          </div>
          ...
          <div className="pb-10">
            ...
            <p className="mt-2 text-lg font-black tracking-[-0.025em] text-white/70">
              {item.headline || item.activity}
            </p>

            {item.currentFocus && (
              <p className="mt-2 text-sm text-white/35">{item.currentFocus}</p>
            )}
          </div>
        </div>
      ))}
...
function StateValue({
  ...
  return (
    <div>
      ...
      {detail && <p className="mt-1 text-xs text-white/25">{detail}</p>}
    </div>
  );
}
      ...
      <section className="relative overflow-hidden py-28">
        ...
        <Container className="relative">
          ...
          <BodyText className="mt-8 max-w-2xl">
            This page updates as my current work, thoughts and attention change.
          </BodyText>

          <div className="mt-10 flex items-center gap-3 text-sm text-white/30">
            <span className="size-2 rounded-full bg-white/20" />
            No public status currently active
          </div>
        </Container>
      </section>
    </main>
...
function getRecoveryStatus(score: number): "positive" | "negative" | "neutral" {
  if (score >= 67) {
    return "positive";
  }
...
function getHealthMetricClass(status: "positive" | "negative" | "neutral") {
  if (status === "positive") {
    return "text-[#C6FF32]";
  }
